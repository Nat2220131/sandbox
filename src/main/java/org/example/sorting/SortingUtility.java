...
public class SortingUtility {
    ...
    public static <T extends Comparable<T>> void merge(T[] data, int first, int mid, int last) {
        ...
        // Copy smaller item from each sub array until one of the subarrays is exhausted
        ...
        //copying remaining elements from first subarray, if any
        while(first1 <= last1){
            temp[index] = data[first1];
            first1++;
            index++;
        ...
        //copying remaining elements from second subarray, if any
        while(first2 <= last2){
            temp[index] = data[first2];
            first2++;
            index++;
